...
namespace FreshGUI
{
    class InventoryExpanded : InventoryPage
    {

            public InventoryExpanded(int x, int y, int width, int height) : base(x, y, width, height + Game1.tileSize)
            {
                inventory.capacity = 96;
                inventory.rows = inventory.capacity / 12;
        ...
        {
            //var horseName = ModEntry.instance.Helper.Reflection.GetField<string>(this, "horseName").GetValue();
            var horseName = Game1.player.Name;
            var trashCanLidRotation = ModEntry.instance.Helper.Reflection.GetField<float>(this, "trashCanLidRotation").GetValue();
            var heldItem = ModEntry.instance.Helper.Reflection.GetField<Item>(this, "heldItem").GetValue();
            var hoveredItem = ModEntry.instance.Helper.Reflection.GetField<Item>(this, "hoveredItem").GetValue();
            var hoverTitle = ModEntry.instance.Helper.Reflection.GetField<string>(this, "hoverTitle").GetValue();
            var hoverText = ModEntry.instance.Helper.Reflection.GetField<string>(this, "hoverText").GetValue();
            ...
            foreach (ClickableComponent equipmentIcon in this.equipmentIcons)
            {
                ...
                if (!(name == "Hat"))
                {
                    if (!(name == "Right Ring"))
                    {
                        if (!(name == "Left Ring"))
                        {
                            if (name == "Boots")
                            {
                                if (Game1.player.boots.Value != null)
                                {
                                    ...
                                    (Game1.player.boots.Value as Item).drawInMenu(b, new Vector2((float)equipmentIcon.bounds.X, (float)equipmentIcon.bounds.Y), equipmentIcon.scale);
                                }
                                else
                        ...
                        {
                            b.Draw(Game1.menuTexture, equipmentIcon.bounds, new Rectangle?(Game1.getSourceRectForStandardTileSheet(Game1.menuTexture, 10, -1, -1)), Color.White);
                            (Game1.player.leftRing.Value as Item).drawInMenu(b, new Vector2((float)equipmentIcon.bounds.X, (float)equipmentIcon.bounds.Y), equipmentIcon.scale);
                        }
                        else
                    ...
                    {
                        b.Draw(Game1.menuTexture, equipmentIcon.bounds, new Rectangle?(Game1.getSourceRectForStandardTileSheet(Game1.menuTexture, 10, -1, -1)), Color.White);
                        (Game1.player.rightRing.Value as Item).drawInMenu(b, new Vector2((float)equipmentIcon.bounds.X, (float)equipmentIcon.bounds.Y), equipmentIcon.scale);
                    }
                    else
                        b.Draw(Game1.menuTexture, equipmentIcon.bounds, new Rectangle?(Game1.getSourceRectForStandardTileSheet(Game1.menuTexture, 41, -1, -1)), Color.White);
                }
                ...
                {
                    b.Draw(Game1.menuTexture, equipmentIcon.bounds, new Rectangle?(Game1.getSourceRectForStandardTileSheet(Game1.menuTexture, 10, -1, -1)), Color.White);
                    (Game1.player.hat.Value as Item).drawInMenu(b, new Vector2((float)equipmentIcon.bounds.X, (float)equipmentIcon.bounds.Y), equipmentIcon.scale, 1f, 0.866f, false);
                }
                else
